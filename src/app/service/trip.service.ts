...
export class TripService {
  ...
  public generateReport(report:TripReport): Observable<any>{
    ...
    return this.http.post<any>(ServiceConstants.tripReportUrl, report,{headers});
  }
  public createNewTrip(createTrip:CreateTripType): Observable<any>{
    const headers = { 'Authorization': 'Bearer '+sessionStorage.getItem("token") }
    return this.http.post<any>(ServiceConstants.newTripUrl, createTrip,{headers});
  }
  public updateTripToDb(currentUpdateTrip:UpdateTripType): Observable<any>{
    const headers = { 'Authorization': 'Bearer '+sessionStorage.getItem("token") }
    return this.http.post<any>(ServiceConstants.updateTripUrl, currentUpdateTrip,{headers}); 
  }
}
